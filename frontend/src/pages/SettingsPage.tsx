    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-4xl space-y-4 p-5">
        ...
        <Panel title="AI provider" icon={<Cpu className="h-3.5 w-3.5" aria-hidden />} bodyClassName="space-y-3 p-4">
          {!geminiConfigured ? (
            <InlineAlert kind="warning" title="Gemini not configured">
              Add <code className="mono">GEMINI_API_KEY</code> to <code className="mono">.env</code> and restart the
              backend. Until then the deterministic rule-based provider keeps the pipeline usable, and the note is marked
              accordingly.
            </InlineAlert>
          ) : (
            <InlineAlert kind="success" title="Gemini key configured">
              Structured clinical output is generated by <code className="mono">{String(status?.ai.model)}</code>.
            </InlineAlert>
          )}
        ...
        <Panel title="Pipeline providers" bodyClassName="grid gap-3 p-4 sm:grid-cols-2">
          <ProviderCard
            ...
            detail="Faster-Whisper adapter available via requirements-asr.txt (ASR_PROVIDER=faster_whisper)."
          />
          <ProviderCard
            title="Diarization"
            name={status?.providers.diarization.name ?? '—'}
            mock={Boolean(status?.providers.diarization.mock)}
            detail="pyannote.audio adapter available (DIARIZATION_PROVIDER=pyannote + HUGGINGFACE_TOKEN)."
          />
          <ProviderCard
            title="Terminology"
        ...
        <InlineAlert kind="info" title="Privacy and scope">
          <ul className="mt-1 list-disc space-y-0.5 pl-4">
            <li>Simulation and training use only. Never enter real patient data.</li>
