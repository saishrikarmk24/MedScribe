  ...
  const handleAddVitals = async (vitalsSummary: string, medsSummary: string) => {
    ...
    try {
      ...
      if (vitalsSummary) {
        ...
        changes.physical_examination = existing ? `${existing}. Vitals: ${vitalsSummary}` : `Vitals: ${vitalsSummary}`
      }
      if (medsSummary) {
        const existing = content.current_medication?.text?.trim() || ''
        changes.current_medication = existing ? `${existing}. Medications: ${medsSummary}` : medsSummary
      }
