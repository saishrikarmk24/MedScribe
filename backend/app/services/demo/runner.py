...
class DemoSimulationRunner:
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            logger.exception("demo_simulation_failed", extra={"session_id": self.runtime.session_id})
            await self.pipeline._emit_error(  # noqa: SLF001 - intentional internal notification
                self.runtime,
                "DEMO_RUNNER_FAILED",
                "The simulation driver stopped unexpectedly. Transcript and note remain available.",
                self.runtime.stage,
            )
